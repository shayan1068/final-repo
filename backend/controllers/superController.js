    ...
    async (req, res, next) => {
        try {
            ...
            for (const file of req.files) {
                files.push({ url: 'http://localhost:3000' + '/images/jobPost/' + folderName + '/' + file.filename });
            }
...
exports.jobRequest = catchAsyncErrors(
    async (req, res, next) => {
        ...
        try {
            ...
            const files = [];
            for (const file of req.files) {
                files.push({ url: 'http://localhost:3000' + '/images/jobRequest/' + folderName + '/' + file.filename });
            }
